...
st.caption("Data GFS (0.25°) dari NOMADS NOAA - Realtime via OpenDAP")
...
if st.sidebar.button("🔎 Tampilkan Visualisasi"):
    ...
    if is_contour:
        cs = ax.contour(var.lon, var.lat, var.values, levels=15, colors='black', linewidths=0.6)
    ...
    # Info update
    update_time = ds.time[0].values
    st.caption(f"🕒 Dataset GFS terakhir: {pd.to_datetime(str(update_time)).strftime('%d %b %Y %H:%M UTC')}")
